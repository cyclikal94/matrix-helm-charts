...
Default image/app version tracks upstream image/git tag `v0.2511.0`.
...
```bash
helm dependency build ./charts/mautrix-gvoice
helm upgrade --install mautrix-gvoice ./charts/mautrix-gvoice -f mautrix-gvoice-values.yaml
```

Install from published OCI registry (preferred):

```bash
helm upgrade --install mautrix-gvoice oci://ghcr.io/cyclikal94/matrix-helm-charts/mautrix-gvoice -n mautrix-gvoice --create-namespace --values mautrix-gvoice-values.yaml
```
...
helm repo update
helm upgrade --install mautrix-gvoice matrix-helm-charts/mautrix-gvoice -n mautrix-gvoice --create-namespace --values mautrix-gvoice-values.yaml
```
...
- ConfigMap: `<release>-mautrix-gvoice-registration`
- Key: `appservice-registration-gvoice.yaml`
...
synapse:
  appservices:
    - configMap: <release>-mautrix-gvoice-registration
      configMapKey: appservice-registration-gvoice.yaml
```
...
- `config.networkExtra`: bridge-specific config merged under top-level `network`. See upstream connector example in [`mautrix/gvoice`](https://github.com/mautrix/gvoice/blob/main/pkg/connector/example-config.yaml).
...
- `values.matrix.example.yaml`: recommended Matrix/ESS-focused mautrix-gvoice config example.
...
```bash
helm dependency build ./charts/mautrix-gvoice
helm lint ./charts/mautrix-gvoice -f ./charts/mautrix-gvoice/values.example.yaml
```

## Verify

```bash
kubectl get pods,svc -l app.kubernetes.io/instance=mautrix-gvoice -n gvoice
kubectl get configmap <release>-mautrix-gvoice-registration -n gvoice
kubectl get configmap <release>-mautrix-gvoice-registration -n <synapse-namespace>
```

## Docs

- [Bridge setup with Docker](https://docs.mau.fi/bridges/general/docker-setup.html?bridge=gvoice)
- [Initial bridge config](https://docs.mau.fi/bridges/general/initial-config.html#mautrix-gvoice)
- [Registering appservices](https://docs.mau.fi/bridges/general/registering-appservices.html)
- [mautrix-gvoice repository](https://github.com/mautrix/gvoice)
