...
```bash
helm dependency build ./charts/mautrix-gmessages
...
```bash
helm upgrade --install mautrix-gmessages oci://ghcr.io/cyclikal94/matrix-helm-charts/mautrix-gmessages -n mautrix-gmessages --create-namespace --values mautrix-gmessages-values.yaml
```
...
- `config.networkExtra`: bridge-specific config merged under top-level `network`. See upstream connector example in [`mautrix/gmessages`](https://github.com/mautrix/gmessages/blob/main/pkg/connector/example-config.yaml).
