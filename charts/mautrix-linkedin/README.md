...
Default image/app version tracks upstream image/git tag `v0.2602.0`.
...
```bash
helm dependency build ./charts/mautrix-linkedin
helm upgrade --install mautrix-linkedin ./charts/mautrix-linkedin -f mautrix-linkedin-values.yaml
```

Install from published OCI registry (preferred):

```bash
helm upgrade --install mautrix-linkedin oci://ghcr.io/cyclikal94/matrix-helm-charts/mautrix-linkedin -n mautrix-linkedin --create-namespace --values mautrix-linkedin-values.yaml
```
...
helm repo update
helm upgrade --install mautrix-linkedin matrix-helm-charts/mautrix-linkedin -n mautrix-linkedin --create-namespace --values mautrix-linkedin-values.yaml
```
...
- ConfigMap: `<release>-mautrix-linkedin-registration`
- Key: `appservice-registration-linkedin.yaml`
...
synapse:
  appservices:
    - configMap: <release>-mautrix-linkedin-registration
      configMapKey: appservice-registration-linkedin.yaml
```
...
- `config.networkExtra`: bridge-specific config merged under top-level `network`. See upstream connector example in [`mautrix/linkedin`](https://github.com/mautrix/linkedin/blob/main/pkg/connector/example-config.yaml).
...
- `values.matrix.example.yaml`: recommended Matrix/ESS-focused mautrix-linkedin config example.
...
```bash
helm dependency build ./charts/mautrix-linkedin
helm lint ./charts/mautrix-linkedin -f ./charts/mautrix-linkedin/values.example.yaml
```

## Verify

```bash
kubectl get pods,svc -l app.kubernetes.io/instance=mautrix-linkedin -n linkedin
kubectl get configmap <release>-mautrix-linkedin-registration -n linkedin
kubectl get configmap <release>-mautrix-linkedin-registration -n <synapse-namespace>
```

## Docs

- [Bridge setup with Docker](https://docs.mau.fi/bridges/general/docker-setup.html?bridge=linkedin)
- [Initial bridge config](https://docs.mau.fi/bridges/general/initial-config.html#mautrix-linkedin)
- [Registering appservices](https://docs.mau.fi/bridges/general/registering-appservices.html)
- [mautrix-linkedin repository](https://github.com/mautrix/linkedin)
