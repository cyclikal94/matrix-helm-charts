...
```bash
helm dependency build ./charts/mautrix-zulip
helm upgrade --install mautrix-zulip ./charts/mautrix-zulip -f mautrix-zulip-values.yaml
```

Install from published OCI registry (preferred):

```bash
helm upgrade --install mautrix-zulip oci://ghcr.io/cyclikal94/matrix-helm-charts/mautrix-zulip -n mautrix-zulip --create-namespace --values mautrix-zulip-values.yaml
```
...
helm repo update
helm upgrade --install mautrix-zulip matrix-helm-charts/mautrix-zulip -n mautrix-zulip --create-namespace --values mautrix-zulip-values.yaml
```
...
- ConfigMap: `<release>-mautrix-zulip-registration`
- Key: `appservice-registration-zulip.yaml`
...
synapse:
  appservices:
    - configMap: <release>-mautrix-zulip-registration
      configMapKey: appservice-registration-zulip.yaml
```
...
- `config.networkExtra`: bridge-specific config merged under top-level `network`. See upstream connector example in [`mautrix/zulip`](https://github.com/mautrix/zulip/blob/main/pkg/connector/example-config.yaml).
...
- `values.matrix.example.yaml`: recommended Matrix/ESS-focused mautrix-zulip config example.
...
```bash
helm dependency build ./charts/mautrix-zulip
helm lint ./charts/mautrix-zulip -f ./charts/mautrix-zulip/values.example.yaml
```

## Verify

```bash
kubectl get pods,svc -l app.kubernetes.io/instance=mautrix-zulip -n zulip
kubectl get configmap <release>-mautrix-zulip-registration -n zulip
kubectl get configmap <release>-mautrix-zulip-registration -n <synapse-namespace>
```

## Docs

- [Bridge setup with Docker](https://docs.mau.fi/bridges/general/docker-setup.html?bridge=zulip)
- [Initial bridge config](https://docs.mau.fi/bridges/general/initial-config.html#mautrix-zulip)
- [Registering appservices](https://docs.mau.fi/bridges/general/registering-appservices.html)
- [mautrix-zulip repository](https://github.com/mautrix/zulip)
