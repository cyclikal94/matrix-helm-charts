...
```bash
helm dependency build ./charts/mautrix-slack
...
```bash
helm upgrade --install mautrix-slack oci://ghcr.io/cyclikal94/matrix-helm-charts/mautrix-slack -n mautrix-slack --create-namespace --values mautrix-slack-values.yaml
```
...
- `config.networkExtra`: bridge-specific config merged under top-level `network`. See upstream connector example in [`mautrix/slack`](https://github.com/mautrix/slack/blob/main/pkg/connector/example-config.yaml).
